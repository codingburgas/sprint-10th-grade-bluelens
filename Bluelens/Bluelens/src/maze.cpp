...
// Draw maze, player, enemies, goal and UI
void DrawMaze(const std::vector<std::vector<Cell>>& maze, int cellSize, const Player& player, const std::vector<Enemy>& enemies, int level)
{
    ...
    for (int y = 0; y < h; ++y)
    {
        for (int x = 0; x < w; ++x)
        {
            ...
            if (maze[y][x].walls[0]) DrawLineEx({ (float)px, (float)py }, { (float)(px + cellSize), (float)py }, thickness, wallColor); // top
            if (maze[y][x].walls[1]) DrawLineEx({ (float)(px + cellSize), (float)py }, { (float)(px + cellSize), (float)(py + cellSize) }, thickness, wallColor); // right
            if (maze[y][x].walls[2]) DrawLineEx({ (float)px, (float)(py + cellSize) }, { (float)(px + cellSize), (float)(py + cellSize) }, thickness, wallColor); // bottom
            if (maze[y][x].walls[3]) DrawLineEx({ (float)px, (float)py }, { (float)px, (float)(py + cellSize) }, thickness, wallColor); // left
        }
    }
    ...
    // draw player
    DrawRectangle(player.x * cellSize + 4, player.y * cellSize + 4, cellSize - 8, cellSize - 8, BLUE);

    // draw enemies
    for (const auto& e : enemies)
    {
        DrawRectangle(e.x * cellSize + 6, e.y * cellSize + 6, cellSize - 12, cellSize - 12, RED);
    }
