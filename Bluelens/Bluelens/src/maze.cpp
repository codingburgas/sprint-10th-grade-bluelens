...
void DrawMaze(const std::vector<std::vector<Cell>>& maze, int cellSize, const Player& player, const std::vector<Enemy>& enemies)
{
    ...
    for (int y = 0; y < (int)maze.size(); ++y)
    {
        for (int x = 0; x < (int)maze[0].size(); ++x)
        {
            ...
            if (maze[y][x].walls[0]) DrawLine(px, py, px + cellSize, py, currentTheme.wallColor);
            if (maze[y][x].walls[1]) DrawLine(px + cellSize, py, px + cellSize, py + cellSize, currentTheme.wallColor);
            if (maze[y][x].walls[2]) DrawLine(px, py + cellSize, px + cellSize, py + cellSize, currentTheme.wallColor);
            if (maze[y][x].walls[3]) DrawLine(px, py, px, py + cellSize, currentTheme.wallColor);
        }
    }
    ...
    {
        DrawRectangle(player.x * cellSize + 3, player.y * cellSize + 3, cellSize - 6, cellSize - 6, currentTheme.playerColor);
    }
    ...
    for (const auto& e : enemies)
    {
        ...
        {
            DrawRectangle(e.x * cellSize + 3, e.y * cellSize + 3, cellSize - 6, cellSize - 6, currentTheme.enemyColor);
        }
    }
}
